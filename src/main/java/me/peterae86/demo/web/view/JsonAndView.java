 ...
 */
package me.peterae86.demo.web.view;
...
public class JsonAndView implements Serializable {
    ...
    public static JsonAndView success() {
    ...
    public static JsonAndView invalidFormatJV() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(600);
        jv.setErrmsg("输入参数格式错误");
        return jv;
    }

    public static JsonAndView invalidParamJV() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(601);
        jv.setErrmsg("输入参数不符合逻辑");
        return jv;
    }
    
    public static JsonAndView orderNotFoundJV() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(651);
        jv.setErrmsg("没有找到符合查询条件的订单");
        return jv;
    }
    
    public static JsonAndView unlockFailureJV() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(652);
        jv.setErrmsg("解锁失败");
    ...
    public static JsonAndView orderIsLocked() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(653);
        jv.setErrmsg("解锁失败");
    ...
    public static JsonAndView failureJV() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        return jv;
    }
    
    public static JsonAndView illegalOrderRequest() {
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(654);
        jv.setErrmsg("非法请求，该订单不属于此用户");
        return jv;
    }

    public static JsonAndView refused(){
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(655);
        jv.setErrmsg("权限不足,你不是管理员");
        return jv;
    }
    
    public static JsonAndView uploadFaild(){
        JsonAndView jv = new JsonAndView();
        jv.setRet(false);
        jv.setErrcode(660);
        jv.setErrmsg("上传失败");
